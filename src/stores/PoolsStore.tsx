...
import { POOL_CONFIG, TOKENS_BY_ASSET_ID, TOKENS_BY_SYMBOL } from '@src/common/constants';
import poolsService from '@src/common/services/poolsService';
...
export default class PoolsStore {
  ...
  setSlippage = (v: number) => !isNaN(v) && v > 1 && v < 100 && (v ^ 0) === v && (this.slippage = v);
  ...
  constructor(rootStore: RootStore, initialState?: ISerializedPoolsStore) {
    ...
    // todo: fix pools request issues, example (http://localhost:3000/undefined/api/v1/pools)
    ...
    reaction(
      ...
      () => Promise.all([this.updateAccountPoolsLiquidityInfo(true), this.updatePoolsState()])
    );
  }
  ...
  setAccountPoolsLiquidity = (v: IShortPoolInfo[] | null, options?: { onlyMain?: boolean; onlyCustom?: boolean }) => {
    if (v == null) {
      this.accountPoolsLiquidity = null;
    } else if (options?.onlyCustom) {
      const mainPoolsInfo = this.accountPoolsLiquidity?.filter(({ pool }) => !pool.isCustom);
      this.accountPoolsLiquidity = mainPoolsInfo != null ? [...mainPoolsInfo, ...v] : v;
    } else if (options?.onlyMain) {
      const customPoolsInfo = this.accountPoolsLiquidity?.filter(({ pool }) => pool.isCustom);
      this.accountPoolsLiquidity = customPoolsInfo != null ? [...customPoolsInfo, ...v] : v;
    } else {
  ...
  @action.bound setAccountPoolsLiquidityLoading = (state: boolean) => (this.accountPoolsLiquidityLoading = state);
  ...
  usdnRate = (assetId: string, coefficient = 1): BN | null => {
    ...
    const pool = this.pools.find(({ tokens }) => tokens.some((t) => t.assetId === assetId));
    ...
    if (pool.currentPrice(assetId, puzzle) == null && pool.tokens.some((t) => t.assetId === puzzle)) {
      return new BN(startPrice ?? 0);
    }
    if (pool.currentPrice(assetId, usdn) == null && pool.tokens.some((t) => t.assetId === usdn)) {
      return new BN(startPrice ?? 0);
    }
  ...
  syncCustomPools = async () => {
    const configs = await poolsService.getPools();
