...
import { IPoolConfig, IPoolConfigStatistics, IToken } from '@src/common/constants';
import { makeAutoObservable } from 'mobx';
import BN from '@src/common/utils/BN';
import tokenLogos from '@src/common/constants/tokenLogos';
import nodeService from '@src/common/services/nodeService';
import { getStateByKey } from '@src/common/utils/getStateByKey';
...
class Pool implements IPoolConfig {
  ...
  public setStatistics = (statistics: IPoolConfigStatistics) => (this.statistics = statistics);
  ...
  setGlobalLiquidityByUSDN = (value: BN | null) => (this.globalLiquidityByUSDN = value);

  public globalLiquidityByPUZZLE: BN | null = null;

  setGlobalLiquidityByPUZZLE = (value: BN | null) => (this.globalLiquidityByPUZZLE = value);

  public get globalLiquidity(): BN {
    if (this.globalLiquidityByUSDN != null) return this.globalLiquidityByUSDN;
    if (this.globalLiquidityByPUZZLE != null && this.puzzleRate.gt(0)) {
  ...
  syncLiquidity = (data?: IData[]) => {
    ...
    if (usdnAsset && usdnLiquidity) {
      globalLiquidityByUSDN = new BN(usdnLiquidity).div(usdnAsset.share).times(100).div(1e6);
    } else if (puzzleAsset && puzzleLiquidity) {
      globalLiquidityByPuzzle = new BN(puzzleLiquidity).div(puzzleAsset.share).times(100).div(1e8);
    }
  ...
  getAccountLiquidityInfo = async (user: string): Promise<IShortPoolInfo> => {
    ...
    const { globalEarnedByOwner } = parsedNodeResponse;
    this.setGlobalEarnedByOwner(globalEarnedByOwner);
    const { addressIndexStaked } = parsedNodeResponse;
    const { globalIndexStaked } = parsedNodeResponse;
    const { globalPoolTokenAmount } = parsedNodeResponse;
    ...
    const liquidityInUsdn = this.globalLiquidity.times(addressIndexStaked).div(globalIndexStaked);
    const percent = liquidityInUsdn.times(new BN(100)).div(this.globalLiquidity);
  ...
  getAccountLiquidityInfoByState = (user: string, state: IData[]): IShortPoolInfo => {
    ...
    const globalPoolTokenAmount = new BN(getStateByKey(state, 'globalPoolTokenAmount') ?? 0);
  ...
  contractKeysRequest = (keys: string[] | string) => nodeService.nodeKeysRequest(this.contractAddress, keys);
}
